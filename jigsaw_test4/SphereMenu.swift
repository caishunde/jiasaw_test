...
{
    //起始角度, 正右为0, CGFloat(M_PI) 代表180度
    let angular:CGFloat = CGFloat(M_PI) + CGFloat(M_PI_2)
    
    //间隔角度
    let kAngleOffset:CGFloat = CGFloat(M_PI_4)
    ...
    required init(startPoint:CGPoint, startImages:Array<UIImage>, submenuImages:Array<UIImage>, tapToDismiss:Bool)
    {
        ...
        self.start = UIImageView(image: startImages[0], highlightedImage: nil)
        ...
        self.bounds = CGRectMake(0, 0, startImages[0].size.width, startImages[0].size.height);
    ...
    func bringItemsToFront()
    {
        ...
        {
            superview?.bringSubviewToFront(items![i])
        }
    ...
    {
        //极坐标形式
        let firstAngle:CGFloat = CGFloat(M_PI_4) - angular + CGFloat(M_PI) + (CGFloat(M_PI_2) - kAngleOffset) + CGFloat(index) * kAngleOffset
